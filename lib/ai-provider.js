...
class AIProvider {
  constructor(config = {}) {
    // Support both "provider" and "defaultProvider" config keys
    this.defaultProvider = config.provider || config.defaultProvider || 'openclaw';
    this.providers = {};
    ...
    // Apply flat config keys (apiKey, endpoint, visionModel) to default provider
    const flat = {};
    if (config.apiKey) flat.apiKey = config.apiKey;
    if (config.endpoint) flat.endpoint = config.endpoint;
    if (config.visionModel) flat.visionModel = config.visionModel;
    if (config.chatModel) flat.chatModel = config.chatModel;
    if (config.embeddingModel) flat.embeddingModel = config.embeddingModel;
    if (Object.keys(flat).length && this.providers[this.defaultProvider]) {
      Object.assign(this.providers[this.defaultProvider], flat);
    }
  ...
  async _fetch(url, body, headers = {}) {
    ...
    try {
      ...
      if (e.cause?.code === 'ECONNREFUSED') {
        throw new Error(`Cannot connect to ${new URL(url).origin} — is the service running?`);
      }
...
class OpenClawAdapter extends BaseAdapter {
  constructor(config) {
    ...
    // OpenClaw may use its own auth token or no auth for local gateway
