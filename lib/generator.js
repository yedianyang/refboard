...
function loadMetadata(inputDir) {
  ...
  if (existsSync(metadataPath)) {
    try {
      ...
      // Silently fall through to default — CLI layer handles user-facing output
    }
  }
  return { items: [], board: {} };
}
...
/**
 * Render board HTML string without writing to disk.
 * @param {object} opts
 * @param {string} opts.inputDir - Project directory
 * @param {string} [opts.outputFile] - Output file path (used for relative image paths)
 * @param {string} [opts.title] - Board title fallback
 * @param {boolean} [opts.embedImages] - Embed images as base64
 * @param {string} [opts.imageBaseUrl] - Base URL prefix for images (e.g. "/images/")
 * @param {string} [opts.homeUrl] - Override home URL
 * @param {object} [opts.config] - Layout config
 * @returns {{ html: string, itemCount: number, boardId: string }}
 */
export function renderBoard({ inputDir, outputFile, title, embedImages, imageBaseUrl, homeUrl, config = {} }) {
  ...
  let items = images.map((img, index) => {
    ...
    let src;
    if (embedImages) {
      src = toBase64DataUrl(img.path);
    } else if (imageBaseUrl) {
      src = imageBaseUrl + encodeURIComponent(img.filename);
    } else if (outputFile) {
      src = relative(dirname(outputFile), img.path);
    } else {
      src = img.filename;
    }
  ...
  // Home page path
  const resolvedHomeUrl = homeUrl || `file://${join(homedir(), '.refboard', 'home.html')}`;
  ...
  let html = template
    ...
    .replaceAll('{{HOME_URL}}', resolvedHomeUrl)
    .replaceAll('{{GENERATED_AT}}', new Date().toISOString());

  return { html, itemCount: items.length, boardId };
}

export async function generateBoard({ inputDir, outputFile, title, embedImages, config = {} }) {
  const result = renderBoard({ inputDir, outputFile, title, embedImages, config });
  writeFileSync(outputFile, result.html, 'utf-8');
  return { itemCount: result.itemCount, boardId: result.boardId };
}
...
function savePositions(inputDir, positionsById) {
  ...
  for (const [key, pos] of Object.entries(positionsById)) {
    // Support both numeric IDs (from template export) and filename keys
    let filename;
    const idx = parseInt(key);
    if (!isNaN(idx) && images[idx]) {
      filename = images[idx].filename;
    } else {
      // Key is a filename directly
      filename = key;
    }

    let item = metadata.items.find(i => i.file === filename);
    if (!item) {
      item = { file: filename };
