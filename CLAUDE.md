...
自动测试无法覆盖视觉和交互，以下变更 commit 后必须标记 `🔍 需人工验证`：
- CSS/样式修改
- Canvas 渲染逻辑（cards, selection, groups, minimap）
- 面板布局变更
- 键盘快捷键变更
- 动画/过渡效果

**Agent 的职责：** commit message 加 `🔍`，并在 SendMessage 中说明需要验证什么、怎么触发。
**Jingxi 的职责：** `npm run tauri dev` 启动后目视+操作确认。
